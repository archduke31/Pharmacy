...
$getCid=mysqli_query("SELECT customer_id FROM prescription WHERE invoice_id='{$invNo}' ",$con);
$getName=mysqli_query("SELECT customer_name FROM invoice WHERE invoice_id='{$invNo}' ",$con;
...
$details=mysqli_fetch_array($getCid);
	...
	$sqlP=mysqli_query("INSERT INTO receipts(reciptNo,customer_id,total,payType,serialno,served_by)
				VALUES('{$recNo}','{$details['customer_id']}','{$amount}','{$pType}','{$serial}','{$_SESSION['username']}')  ",$con);
