...
$getPresid=mysqli_query("SELECT 1+MAX(prescription_id) FROM prescription",$con);
...
while($item12=mysqli_fetch_array($getDetails))
			{	
			$getDetails12=mysqli_query("SELECT stock_id, cost FROM stock WHERE drug_name='{$item12['drug_name']}'",$con);	
			...
			$sqlIp=mysqli_query("INSERT INTO prescription_details(pres_id,drug_name,strength,dose,quantity)
				VALUES('{$presIdd}','{$details2['stock_id']}','{$item12['strength']}','{$item12['dose']}','{$item12['quantity']}') ",$con);	
					
			}
		$sqlD=mysqli_query("DELETE FROM tempprescri WHERE customer_id='{$c_id}' ",$con);
