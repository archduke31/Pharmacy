...
if(isset($_POST['drug_name']))
{
	...
	$sql=mysqli_query("INSERT INTO tempPrescri(customer_id,customer_name,age,sex,postal_address,phone,drug_name,strength,dose,quantity)
						VALUES('{$_SESSION['custId']}','{$_SESSION['custName']}','{$_SESSION['age']}','{$_SESSION['sex']}','{$_SESSION['address']}','{$_SESSION['phone']}','{$drug}','{$strength}','{$dose}','{$quantity}')",$con);
						
						$_SESSION['quantity']=$quantity;
	$get_cost=mysqli_query("SELECT cost FROM stock WHERE drug_name='{$drug}'",$con);
	...
	echo "<table width=\"100%\" border=1>";
        ...
        // loop through results of database query, displaying them in the table
		 $result = mysqli_query("SELECT * FROM tempPrescri",$con)or die(mysqli_error());
...
$getDetails=mysqli_query("SELECT invoice,drug,cost,quantity FROM invoice_details WHERE invoice='{$invoice}'",$con);
$getQuantity=mysqli_query("SELECT quantity FROM invoice_details WHERE invoice_id='{$invoice}'",$con);
...
while($item5=mysqli_fetch_array($getDetails))
			{
			$getDrug=mysqli_query("SELECT drug_name FROM stock WHERE stock_id='{$item5['drug']}'",$con);
